...
并发版的每条候选对在 `_process_single_candidate` 中经历以下管道（每个线程独立执行）：

```
构建判断 Prompt ──► 调用判断 LLM ──► 解析判断 JSON
       │                                      │
       │           ┌──── 重试 ────────────────┘
       │           ▼
       │     构建评审 Prompt ──► 调用评审 LLM ──► 解析评审 score
       │                                      │
       │                    ┌── score >= 70 ──► 通过 → QA 对
       │                    │
       └── 重试次数 < 2 ────┤
...
- **判断**：LLM 对条款对输出五级标签 + 置信度 + 证据链
- **评审**：另一个 LLM 调用校验判断结果的格式完整性和逻辑一致性，给出 0-100 分
- **重试**：分数 < 70 或 JSON 不完整时，将评审反馈注入下一轮判断 Prompt，最多重试 2 次
- **过滤**：标签为"无关"的直接丢弃，不进入后续流程
- **冲突生成**：对"高度符合"/"基本一致"的 pair，额外调用 LLM 构造冲突场景 QA

`node_batch_process` 用 `ThreadPoolExecutor(max_workers=20)` + `wait(FIRST_COMPLETED)` 动态调度：完成一条立即提交下一条，保持 20 条始终在飞。每 5000 条保存一次 JSON 断点，中断后可从断点续传。

### 实体解析

条款在进入候选对筛选前，需要从文本中提取结构化实体。内外规采用不同策略：

| | 内规 (~7k 条) | 外规 (~134k 条) |
|---|---|---|
| 方式 | LLM 逐条提取 | 关键词匹配 |
| 精度 | 高（LLM 理解上下文） | 中（基于内规词典命中） |
| 原因 | 数量少，可承受 LLM 调用 | 数量大，避免 134k 次 LLM 调用 |

**提取的实体类型：**

| 要素 | 说明 | 示例 |
|------|------|------|
| subjects | 法律主体（行为方/被约束方） | 商业银行、借款人、董事会 |
| objects | 法律客体（业务对象） | 贷款、个人信息、不良资产 |
| biz_dims | 业务维度（所属领域） | 信贷管理、反洗钱、公司治理 |
| action | 行为模态 | 应当、不得、可以、禁止 |

**处理流程：**

```
内规 DOCX ──► 文本清洗 ──► LLM 提取实体 ──► 同义词归一化 ──► 内规关键词表
                                                       │
                                                       ▼
外规 JSON ──► 文本清洗 ──► 关键词匹配 ◄──────── 关键词过滤（剔除短词 + 黑名单）
```

- **文本清洗**（`text_cleaner.py`）：去除 HTML 标签、HYPERLINK 残留，规范化空白
- **LLM 提取**（`semantics_extractor.py`）：内规逐条调用 LLM，输出 subjects/objects/biz_dims/action JSON
- **同义词归一化**（`synonym_normalizer.py`）：将不同表述映射为规范名。例如 "本行""该银行" → "商业银行"，"信贷""放款" → "贷款"，"信贷业务""贷款管理" → "信贷管理"
- **关键词匹配**（`nodes_concurrent.py`）：从内规实体构建关键词表，对外规条款文本做子串匹配（`"信贷管理" in clause_text`）
- **关键词过滤**（`_filter_keywords`）：剔除 <3 字符的短词和 30+ 个泛化黑名单词（如"银行""业务""管理""人员"），避免命中率过高

内规实体提取结果写入 `output/inner_clauses_with_semantics.json` 缓存，下次运行跳过 LLM 调用直接加载。
